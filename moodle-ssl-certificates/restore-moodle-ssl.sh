...
# Скрипт восстановления SSL сертификатов для Moodle из репозитория
...
echo "=== Восстановление SSL сертификатов Moodle из репозитория ==="
echo "📅 Дата: $(date)"
echo "🌐 Домен: $DOMAIN"
...
if [ ! -d "$CERT_DIR" ]; then
    ...
    echo "   Сначала создайте резервную копию: sudo ./backup-moodle-ssl.sh"
    exit 1
fi
...
echo "6. Обновление конфигурации Nginx для Moodle..."

# Проверка и обновление конфигурации Nginx
NGINX_CONFIG="/etc/nginx/sites-available/moodle-ssl"

if [ ! -f "$NGINX_CONFIG" ]; then
    echo "   📝 Создание SSL конфигурации Nginx для Moodle..."
    
    cat > "$NGINX_CONFIG" << EOF
# SSL конфигурация для Moodle ($DOMAIN)
...
EOF
    ...
    echo "   ✅ SSL конфигурация Nginx для Moodle создана и активирована"
else
    echo "   ✅ SSL конфигурация Nginx для Moodle уже существует"
fi
...
echo
echo "✅ Восстановление SSL сертификатов для Moodle завершено успешно!"
echo
echo "📋 Информация о установленном сертификате:"
openssl x509 -in "$LETSENCRYPT_DIR/cert.pem" -noout -subject -dates
echo
echo "🌐 Проверьте сайт Moodle: https://$DOMAIN"
...
echo
echo "🎓 Конфигурация Moodle:"
echo "   Обновите config.php: \$CFG->wwwroot = 'https://$DOMAIN';"
echo "   SSL редирект настроен автоматически"
