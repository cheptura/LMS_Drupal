...
# Скрипт резервного копирования SSL сертификатов
...
echo "=== Резервное копирование SSL сертификатов ==="
echo "📅 Дата: $(date)"
echo "🌐 Домен: $DOMAIN"
...
if cp "$LETSENCRYPT_DIR/chain.pem" "$CERT_DIR/" 2>/dev/null; then
    ...
    echo "   ❌ Ошибка копирования chain.pem"
fi
...
echo
echo "💡 Следующие шаги:"
echo "   1. Добавьте файлы в git: git add ssl-certificates/"
echo "   2. Закоммитьте: git commit -m 'Обновлены SSL сертификаты для $DOMAIN'"
