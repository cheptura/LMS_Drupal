...
# Причина: Расширение PHP cURL не установлено или не загружено правильно

# 🎯 РЕШЕНИЕ: Запустите пост-установку, она автоматически исправит проблему
cd /путь/к/moodle-installation
sudo ./09-post-install.sh

# Скрипт автоматически:
# - Переустановит cURL и все зависимости
# - Проверит загрузку модуля в PHP
# - Добавит extension=curl в конфигурацию если нужно
# - Протестирует работу cURL с реальным запросом

# 🎯 ЭКСТРЕННОЕ РУЧНОЕ ИСПРАВЛЕНИЕ:
sudo apt update
sudo apt install -y curl libcurl4-openssl-dev ca-certificates
sudo apt install -y --reinstall php8.3-curl php8.3-common
echo "extension=curl" | sudo tee -a /etc/php/8.3/fpm/php.ini
echo "extension=curl" | sudo tee -a /etc/php/8.3/cli/php.ini
sudo systemctl restart php8.3-fpm

# Проверка: php -r "echo function_exists('curl_exec') ? '✅ OK' : '❌ FAIL';"

# ℹ️ ПРИМЕЧАНИЕ: Предупреждение "Module curl is already loaded" - НОРМАЛЬНО!
# Это означает что cURL загружен дважды, но функция работает корректно
```
...
# 🎯 РЕШЕНИЕ: Запустите обновленный скрипт безопасности
cd /путь/к/moodle-installation
sudo ./10-security.sh

# Скрипт автоматически:
# - Удалит устаревшие конфигурации с проблемными директивами
# - Создаст новые конфигурации только со стандартными директивами Nginx
# - Протестирует конфигурацию перед применением
...
```bash
# ✅ ИСПРАВЛЕНО в версии скриптов от 6 сентября 2025!
# Теперь 10-security.sh правильно настраивает DDoS защиту

# ❌ Старая проблема: "location directive is not allowed here in /etc/nginx/conf.d/ddos-protection.conf"
# Проявления: 
# - nginx: configuration file /etc/nginx/nginx.conf test failed
# - Ошибка при проверке Nginx конфигурации
# - Сайт может работать, но конфигурация некорректна

# Причина: В старых версиях скрипта location блоки создавались в conf.d файле

# 🎯 РЕШЕНИЕ: Используйте обновленную версию скрипта
...
# - Добавляет rate limiting зоны в nginx.conf (http блок)
# - Добавляет location блоки в конфигурацию сайта (server блок)
# - Правильно тестирует конфигурацию перед применением

# 🔧 Если проблема уже возникла (ручное исправление):
