...
⚠️ **ВАЖНО для SSL сертификатов:** Скрипты автоматически проверяют наличие сохраненных сертификатов в репозитории и восстанавливают их при необходимости. Новые сертификаты автоматически сохраняются в `../moodle-ssl-certificates/`
...
# Решение 1: ИСПОЛЬЗОВАТЬ СОХРАНЕННЫЕ СЕРТИФИКАТЫ (автоматически)
# Скрипт 05-configure-ssl.sh автоматически проверяет наличие сертификатов в репозитории
# Если найдены действующие сертификаты, они восстанавливаются без обращения к Let's Encrypt

# Решение 2: РУЧНОЕ ВОССТАНОВЛЕНИЕ СЕРТИФИКАТОВ
cd /path/to/LMS_Drupal
sudo ./moodle-ssl-certificates/restore-moodle-ssl.sh

# Решение 3: ПОДОЖДАТЬ (если нет сохраненных сертификатов)
...
# Решение 4: ИСПОЛЬЗОВАТЬ STAGING СЕРТИФИКАТЫ (для тестирования)
...
# Решение 5: ПРОДОЛЖИТЬ БЕЗ SSL (временно)
...
sudo ./moodle-ssl-certificates/backup-moodle-ssl.sh
