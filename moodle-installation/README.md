...
## Описани### 🛠️ Утилиты ### 🛠️ Утилиты администрирования:
...
- **fix-php-versions.sh** - Исправление проблем с версиями PHP (устанавливает только PHP 8.2)истрирования:
- **update-moodle.sh** - Обновление Moodle до новых версий
- **backu#### Ошибка "php8.2-fmp.service not found"
```bash
# Скрипты теперь автоматически определяют доступную версию PHP
# Просто повторите установку:
sudo ./02-install-webserver.sh

# Или проверьте текущую версию PHP:
php --version
...
- **diagnose-moodle.sh** - Полная диагностика системы Moodle
- **fix-config-issues.sh** - Исправление проблем конфигурации
- **diagnose-php-fpm.sh** - Диагностика и исправление PHP-FPMтоматизированные скрипты для установки Moodle 5.0+ на Ubuntu 24.04 ### 📁 Важные директории:
...
#### Ошибка "php8.2-fmp.service not found" или проблемы с версиями PHP
```bash
...
php --version
sudo systemctl start php8.1-fmp  # или другую доступную версию
